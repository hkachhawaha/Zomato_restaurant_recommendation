...
You are tasked with building an AI-powered restaurant recommendation service inspired by Zomato. The system should intelligently suggest restaurants based on user preferences by combining structured data with Gemini 2.5 Flash as the Large Language Model (LLM).
Objective
Design and implement an application that:
Takes user preferences (such as location, budget, cuisine, and ratings)
Uses a real-world dataset of restaurants
Leverages Gemini 2.5 Flash to generate personalized, human-like recommendations
...
Manage LLM API credentials securely via a local `.env` configuration file
